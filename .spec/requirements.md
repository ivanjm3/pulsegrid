...
### Requirement 19: Analytics Pipeline — Job Lifecycle Event Streaming

**User Story:** As a platform engineer, I want job lifecycle events streamed into an analytics store and surfaced on a dashboard, so that I can diagnose pipeline bottlenecks, track throughput trends, and demonstrate end-to-end data engineering depth.

#### Acceptance Criteria

1. WHEN a Worker_Pod starts, completes, or fails a job, THE pod SHALL publish a `Job_Lifecycle_Event` message to the Kafka topic `job-lifecycle-events` with fields: job_id, event_type (job_started | job_completed | job_failed | rendition_completed), rendition_id (nullable), error_class (nullable: retryable | permanent | constraint), error_reason (nullable), pod_id, timestamp
2. THE `job-lifecycle-events` topic SHALL have 8 partitions, replication factor 3 (production), and 30-day retention to support replay and backfill
3. THE Analytics_Consumer service SHALL maintain a dedicated Kafka consumer group (`pulsegrid-analytics`) reading from `job-lifecycle-events`; it SHALL NOT share a consumer group with Worker_Pods, ensuring worker throughput is unaffected
4. WHEN the Analytics_Consumer processes a `Job_Lifecycle_Event`, THE consumer SHALL upsert into the `analytics.job_lifecycle_events` table (append-only, denormalized) within 5 seconds of the event's Kafka publish timestamp under normal load
5. THE analytics schema SHALL be isolated in a separate Postgres schema (`analytics`) from the operational schema (`public`) to prevent query contention
6. THE system SHALL maintain the following SQL aggregate views refreshed on demand (or via a scheduled job every 60 seconds):
   - `analytics.v_throughput_per_minute`: jobs completed per minute over the last 24 hours
   - `analytics.v_latency_percentiles`: p50, p95, p99 end-to-end latency (job_started → job_completed) by 1-hour bucket over the last 7 days
   - `analytics.v_failure_rate_by_class`: failure count and rate broken down by error_class (retryable/permanent/constraint) per hour over the last 24 hours
   - `analytics.v_rendition_breakdown`: average latency and failure rate per rendition type (720p/480p/hls) over the last 24 hours
7. THE Operational_Dashboard SHALL expose the four views above via a read-only API endpoint `GET /analytics/summary` returning a JSON payload with all four view results
8. THE Grafana dashboard SHALL include an "Analytics" row sourced from the `analytics` schema views, displaying: throughput trend (line chart), latency percentiles (bar chart), failure rate by class (pie/bar), and per-rendition breakdown (table)
9. WHEN the Analytics_Consumer fails to write to the analytics store, THE consumer SHALL NOT commit the Kafka offset, ensuring the event is reprocessed on restart (at-least-once delivery to the analytics sink)
10. THE Analytics_Consumer SHALL expose a `/metrics` endpoint on port 8082 emitting: `pulsegrid_analytics_events_processed_total` (counter, labeled by event_type), `pulsegrid_analytics_sink_lag_seconds` (gauge: wall-clock delay from event timestamp to sink write time), `pulsegrid_analytics_consumer_errors_total` (counter, labeled by error_type)

#### Glossary Additions

- **Job_Lifecycle_Event**: A Kafka message published by a Worker_Pod on each state transition (started, rendition_completed, completed, failed), used exclusively as input to the analytics pipeline
- **Analytics_Consumer**: A dedicated Go service (separate deployment) that reads `job-lifecycle-events` and writes to the `analytics` Postgres schema
- **Analytics_Sink**: The `analytics.job_lifecycle_events` Postgres table — append-only, denormalized, optimized for aggregation
- **Operational_Dashboard**: The existing Grafana dashboard, extended with an Analytics row sourced from the `analytics` schema